...
namespace FbxSharp
{
    public class Converter7400 : IConverter
    {
            ...
            FbxScene scene=null)
        {
            ...
            if (headerExt != null)
            {
                var sceneInfo = headerExt.FindPropertyByName("SceneInfo");
                if (sceneInfo != null)
                {
                    var sceneInfoProps =
                        sceneInfo.FindPropertyByName("Properties70");
                    var docInfo = scene.GetDocumentInfo();
                    ImportProperties(docInfo,
                        ConvertProperties70(sceneInfoProps, state));
                }
            }
            ...
            foreach (var obj in objs.Properties)
            {
                ...
                // Console.WriteLine($"Converted {obj.Name} ({obj.Location}, {obj.Values[0]} ({obj.Values[0].GetType()})) to {fobj}");
                fbxObjects.Add(fobj);
                fbxObjectsById[fobj.UniqueId] = fobj;
            }
            var inFileIdsByActualIds = new Dictionary<ulong, ulong>();
            foreach (var infileId in actualIdsByInFileIds.Keys)
            {
                var actualId = actualIdsByInFileIds[infileId];
                inFileIdsByActualIds[actualId] = infileId;
            }

            // connect objects
            var conns = parsed.FindPropertyByName("Connections");
            CheckConnections(conns);
            var nonDstObjs = new HashSet<FbxObject>(fbxObjects);
            var nonSrcObjs = new HashSet<FbxObject>(fbxObjects);
            foreach (var conn in conns.Properties)
            {
                CheckConnection(conn);
                var connType = ((string)conn.Values[0]);
                var asLong = ((Number)conn.Values[1]).AsLong;
                ...
                {
                case "OO":
                    dstObj = (dstId == 0 ? scene : fbxObjectsById[dstId]);
                    nonDstObjs.Remove(dstObj);
                    srcObj = fbxObjectsById[srcId];
                    nonSrcObjs.Remove(srcObj);
                    dstObj.ConnectSrcObject(srcObj);
                    // Console.WriteLine($"Connected from {fbxObjectsById[srcId]} -> {dstObj}");
                    break;
                case "OP":
...
//                throw new ConversionException(defs.Location, "Explicit count does not match actual number of properties.");
        ...
        public static FbxLayerElementNormal ConvertLayerElementNormal(ParseObject obj, ConversionState state)
        {
            ...
            foreach (var prop in obj.Properties)
            {
                ...
                case "ReferenceInformationType":
                    ...
                    break;
                case "Normals":
                    var dest = normals.GetDirectArray().List;
                    if (prop.Properties.Count > 0)
                    {
                        var source = prop.Properties[0].Values;
                        int i;
                        for (i = 0; i < source.Count; i+=3)
                        {
                            var x = ((Number)source[i + 0]).AsDouble.Value;
                            var y = ((Number)source[i + 1]).AsDouble.Value;
                            var z = ((Number)source[i + 2]).AsDouble.Value;
                            var v = new FbxVector4(x, y, z, 0);
                            dest.Add(v);
                        }
                    }
                    else if (prop.Values.Count > 0)
                    {
                        var source = (double[])prop.Values[0];

                        int i;
                        for (i = 0; i < source.Length; i+=3)
                        {
                            var x = source[i + 0];
                            var y = source[i + 1];
                            var z = source[i + 2];
                        ...
                        throw new ConversionException(prop.Location,
                            "Normal array not found");
                    break;
                case "NormalsW":
                    normalsW.AddRange(
                    ...
                    break;
                case "NormalsIndex":
                    var destindex = normals.GetIndexArray().List;
                    if (prop.Properties.Count > 0)
                    {
                        foreach (var i in prop.Properties[0].Values)
                            destindex.Add((int)i);
                    }
                    else if (prop.Values.Count > 0)
                    {
                        foreach (var i in (int[])prop.Values[0])
                            destindex.Add(i);
                    }
                    else
                        throw new ConversionException(prop.Location,
        ...
        public static FbxLayerElementUV ConvertLayerElementUV(ParseObject obj, ConversionState state)
        {
            ...
            foreach (var prop in obj.Properties)
            {
                ...
                case "ReferenceInformationType":
                    ...
                    break;
                case "UV":
                    var dest = uvs.GetDirectArray().List;
                    if (prop.Properties.Count > 0)
                    {
                        dest.AddRange(
                            prop.Properties[0].Values
                    ...
                    break;
                case "UVIndex":
                    var destIndex = uvs.GetIndexArray().List;
                    if (prop.Properties.Count>0)
                    {
                        destIndex.AddRange(
                            prop.Properties[0].Values
                    ...
                    {
                        var source = (int[])prop.Values[0];
                        foreach (var i in source)
                            destIndex.Add(i);
                    }
                    else
                        throw new ConversionException("UV array not found");
        ...
        public static FbxLayerElementMaterial ConvertLayerElementMaterial(ParseObject obj, ConversionState state)
        {
            ...
            foreach (var prop in obj.Properties)
            {
                ...
                case "ReferenceInformationType":
                    ...
                    break;
                case "Materials":
                    var dest = material.MaterialIndexes.List;
                    if (prop.Properties.Count>0)
                    {
                        dest.AddRange(
                            prop.Properties[0].Values.Select(n =>
                                (int)((Number)n).AsLong.Value));
                    }
                    else if (prop.Values.Count > 0)
                    {
                        foreach (var i in (int[])prop.Values[0])
        ...
        public static List<PropInfo> ConvertProperties70(ParseObject props70, ConversionState state)
        {
            ...
            foreach (var p in props70.Properties)
            {
                ...
                case "KString":
                    ...
                    if (type2 == "Url")
                        propType2 = FbxDataTypes.FbxUrlDT;
                ...
                case "Number":
                    ...
                    break;
                case "DateTime":
                    propType = typeof(FbxDateTime);
                    propType2 = FbxDataTypes.FbxDateTimeDT;
                    propValue = new FbxDateTime();
                    break;
                default:
        ...
        {
            var values = (int[])obj.Values[0]; // .Properties[0].Values;
            int i;
        ...
        public static FbxNode ConvertNode(ParseObject obj, ConversionState state)
        {
            ...
            foreach (var prop in obj.Properties)
            {
                ...
                case "MultiTake":
                    ...
                    break;
                case "Shading":
                    if (prop.Values[0] is bool)
                        node.MultiTake = (bool)prop.Values[0];
                    else
                        node.MultiTake = prop.Values[0].ToString() == "T";
                    break;
                case "Culling":
        ...
        public static void ImportProperty(FbxObject obj, string name, Type type, FbxDataType type2, object value)
        {
            ...
            if (pprop == null || !pprop.IsValid())
                pprop = obj.FindPropertyHierarchical(name, type2);
            if (pprop == null || !pprop.IsValid())
                pprop = FbxProperty.Create(obj, type2, name);
            pprop.Set(value);
        }

        public static void ImportProperties(FbxObject obj, 
            IEnumerable<PropInfo> propinfos)
        {
        ...
        public static FbxMatrix ConvertMatrix(ParseObject obj)
        {
            ...
            if (obj.Properties.Count > 0)
            {
                ...
                if (values.Count != 16)
                    throw new ConversionException(
                        obj.Location,
                        $"Incorrect number of values for FbxMatrix. " +
                        $"Expected 16. Got {values.Count} instead.");

                v = values.Select(n => ((Number)n).AsDouble.Value).ToArray();
            }
            else if (obj.Values.Count > 0)
                v = (double[])obj.Values[0];
            else
                throw new ConversionException("Matrix values not found");
        ...
        public static FbxSurfaceMaterial ConvertMaterial(ParseObject obj, ConversionState state)
        {
            ...
            if (shadingModel != "phong" && shadingModel != "Phong")
        ...
        public static FbxCluster ConvertCluster(ParseObject obj, ConversionState state)
        {
            ...
            bool hasTransformAssociateModel = false;
            ...
            foreach (var prop in obj.Properties)
            {
                ...
                case "Indexes":
                    ...
                    else if (prop.Values.Count > 0)
                    {
                        ...
                        throw new ConversionException(obj.Location,
                            "Cluster indexes array not found");
                    hasIndexes = true;
                    break;
                case "Weights":
                    if (prop.Properties.Count>0)
                    {
                        cluster.ControlPointWeights.AddRange(
                            prop.Properties[0].Values.Select(n =>
                                ((Number)n).AsDouble.Value));
                    }
                    else if (prop.Values.Count > 0)
                    {
                        cluster.ControlPointWeights.AddRange(
                            (double[])prop.Values[0]);
                    }
                    else
                        throw new ConversionException(obj.Location,
                            "Cluster weights not found");
                    hasWeights = true;
                    break;
                case "Transform":
                ...
                case "TransformLink":
                    ...
                    break;
                case "TransformAssociateModel":
                    cluster.SetTransformAssociateModelMatrix(
                        ConvertMatrix(prop));
                    hasTransformAssociateModel = true;
                    break;
                default:
            ...
            foreach (var prop in obj.Properties)
            {
                ...
                case "Properties70":
                    ...
                    foreach (var propinfo in propinfos)
                    {
                        ...
                        {
                            ImportProperty(animCurveNode, pname, ptype, ptype2, pvalue);
                        }
                    }
                    break;
                default:
